...
This project intentionally uses a minimal requirements file:

```txt
streamlit
```

Do not add Python audio packages such as `pydub`, `librosa`, `sounddevice`, `scipy`, or `streamlit-components-lib`. Audio is handled in the browser with the Web Audio API for lower latency and easier deployment.
