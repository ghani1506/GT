...
DEFAULT_NOTES = [
    {"label": "1", "key": "A", "freq": 392.00},
    {"label": "2", "key": "S", "freq": 440.00},
    {"label": "3", "key": "D", "freq": 493.88},
    {"label": "4", "key": "F", "freq": 523.25},
    {"label": "5", "key": "J", "freq": 587.33},
    {"label": "6", "key": "K", "freq": 659.25},
    {"label": "7", "key": "L", "freq": 739.99},
    {"label": "8", "key": ";", "freq": 783.99},
]
...
with st.sidebar:
    ...
    st.write("Upload real caklempong hits for original texture. Files are mapped to notes in order.")
    samples = st.file_uploader(
        "Caklempong audio samples",
        type=["wav", "mp3", "ogg", "m4a"],
        accept_multiple_files=True,
    )
    volume = st.slider("Volume / loudness", 0.0, 2.0, 1.35, 0.01)
...
<style>
  ...
  .keyboard {{
    ...
    margin: 0 auto;
    display: grid;
    grid-template-columns: repeat(4, minmax(var(--pad-size), 1fr));
    gap: var(--gap);
  ...
  @keyframes padVibrate {{
    0% {{ transform: translate3d(0,0,0) scale(1); }}
    12% {{ transform: translate3d(-2px,1px,0) scale(.965); }}
    24% {{ transform: translate3d(2px,-1px,0) scale(.982); }}
    36% {{ transform: translate3d(-1px,-2px,0) scale(.972); }}
    48% {{ transform: translate3d(2px,2px,0) scale(.988); }}
    60% {{ transform: translate3d(-1px,1px,0) scale(.976); }}
    76% {{ transform: translate3d(1px,0,0) scale(.992); }}
    100% {{ transform: translate3d(0,0,0) scale(1); }}
  }}
  @keyframes centerPulse {{
    0% {{ box-shadow: inset 0 8px 10px rgba(255,255,255,.08), inset 0 -9px 13px rgba(0,0,0,.45), 0 0 0 0 rgba(245,197,66,.0); }}
    40% {{ box-shadow: inset 0 8px 12px rgba(255,255,255,.18), inset 0 -9px 13px rgba(0,0,0,.42), 0 0 0 16px rgba(245,197,66,.12); }}
    100% {{ box-shadow: inset 0 8px 10px rgba(255,255,255,.08), inset 0 -9px 13px rgba(0,0,0,.45), 0 0 0 0 rgba(245,197,66,.0); }}
  }}
  .pad.active {{
    animation: padVibrate 135ms linear both;
    filter: brightness(1.35) saturate(1.25);
  }}
  .pad.active::after {{
    animation: centerPulse 180ms ease-out both;
  }}
  .label {{
    z-index: 2;
  ...
  .note {{ font-size: 26px; font-weight: 800; }}
  .key {{ font-size: 13px; opacity: .88; }}
  .hint {{
    max-width: 980px;
    margin: 18px auto 0;
    opacity: .86;
  ...
  @media (max-width: 700px) {{
    .keyboard {{ grid-template-columns: repeat(2, minmax(var(--pad-size), 1fr)); }}
  }}
</style>
</head>
<body>
...
<div class="wrap" id="instrumentWrap">
  <div class="stage-header">
    ...
    <div class="status">
      <div class="pill" id="audioStatus">Tap any pad to unlock audio</div>
      <div class="pill">Air Muleh inspired Brunei motif</div>
      <div class="pill">Keyboard: A S D F / J K L ;</div>
    </div>
  </div>
  <div class="keyboard" id="keyboard"></div>
  <div class="hint">
    For true original caklempong texture, upload clean one-shot caklempong recordings in the sidebar. Without uploads, this app uses a fast metallic gong-like synthesis fallback.
  </div>
</div>
<script>
...
function playSample(index) {{
  ...
  gain.gain.exponentialRampToValueAtTime(1.25, now + 0.004);
...
notes.forEach((n, i) => {{
  ...
  label.innerHTML = `<div class="note">${{n.label}}</div><div class="key">${{n.key}}</div>`;
...
components.html(component_html, height=max(620, pad_size * 2 + 260), scrolling=False)
...
with st.expander("Deployment notes"):
    st.markdown(
        ...
        - Loudness is boosted with a browser-side limiter/compressor to help prevent harsh clipping and distortion.
        """
    )
